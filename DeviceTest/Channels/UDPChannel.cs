...
namespace JK.Channels.UDP
{
    public class UDPClientChannel:BaseChannel
    {
        ...
        {
            this.Caption = Caption + "_UDPClient";
            this.Initialize();
        }
        ...
        public Boolean OpenBoardCast()
        {
            try
            {
                ...
                this.LastErrorMessage = "UDP 广播启动！";
                this.Notify(UDP_CONTROL_EVENT, ChannelControl.Open.ToString(), ChannelResult.OK, this.LastErrorMessage);
                return true;
            }
            catch (System.ObjectDisposedException)
            {
                this.LastErrorMessage = "UDP 启动广播出错！";
            }
            ...
            {
                this.LastErrorMessage = "网络访问出错！";
            }
            this.Notify(UDP_CONTROL_EVENT, ChannelControl.Open.ToString(), ChannelResult.CanNotOpen, this.LastErrorMessage);
            return false;        
        }
        ...
        public override Boolean Open()
        {
            try
            {
                ...
                this.LastErrorMessage = "UDP Client 连接成功！";
                this.Notify(UDP_CONTROL_EVENT,ChannelControl.Open.ToString(), ChannelResult.OK, this.LastErrorMessage);
                return true;
            }
            catch (System.ObjectDisposedException)
            {
                this.LastErrorMessage = "UDP Client是关闭的！";
            }
            catch (System.ArgumentOutOfRangeException)
            {
                this.LastErrorMessage = "端口[" + this.Port.ToString() + "]无效！";
        ...
        public override Boolean Close()
        {
            try
            {
                ...
                this.LastErrorMessage = "UDPClient 关闭！";
                this.Notify(UDP_CONTROL_EVENT, ChannelControl.Close.ToString(), ChannelResult.OK, this.LastErrorMessage);
            }
            catch(System.Net.Sockets.SocketException)
            {
                this.LastErrorMessage = "网络异常！";
                this.Notify(UDP_CONTROL_EVENT, ChannelControl.Close.ToString(), ChannelResult.CanNotClose, this.LastErrorMessage);
                return false;
            }
            return true;
        }
            ...
            while (!this.terminated)
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        ReceiveString = Encoding.Unicode.GetString(ReceiveByte);
